# Defined in /var/folders/rc/nmm020f16db3wnq_h83q2ztr0000gn/T//fish.3mr3QO/cdh.fish @ line 2
function cdh --description 'Menu based cd command'
    ...
    for i in (seq $dirc -1 1)
        ...
        printf '%s %s) %s %s%s%s\n' (set_color $label_color) $letters[$i] (set_color normal) $dir_color $dir $dir_color_reset
    end

    # Ask the user which directory from their history they want to cd to.
    set -l msg (_ 'Select directory by letter: ')
